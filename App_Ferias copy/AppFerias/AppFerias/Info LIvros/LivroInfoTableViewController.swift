    ...
    @IBAction func botaoAdicionar(_ sender: Any) {
        ...
        self.view.endEditing(true)
            if let bookP = NSEntityDescription.insertNewObject(forEntityName: "Book", into: context) as? Book {
                bookP.title = book?.title
                bookP.authors = book?.authors
                bookP.descriptions = book?.descriptions
                bookP.image = book?.image
                bookP.pageCount = book?.pageCount
                bookP.publishedDate = book?.publishedDate
        }
